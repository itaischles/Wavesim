...
def _update_H(Ex, Ey, Ez, Hx, Hy, Hz,
              mu_x, mu_y, mu_z, dt, dxp, dyp, dzp, Nx, Ny, Nz):
    # Every H derivative differences an E field sitting on an integer NODE along the
    # differenced axis, so it is divided by the PRIMARY width there, indexed by the
    # loop counter of that axis (dxp[i]/dyp[j]/dzp[k]). Mirrors update.py's
    # dyp[:-1] broadcast: the output cell index equals the diff's lower index. On a
    # uniform grid the arrays are the exact constant spacing, bit-identical to the old
    # scalar divisor.
    if Nz > 1:
        ...
        for i in prange(Nx):
            for j in range(Ny - 1):
                for k in range(Nz - 1):
                    dEz_dy = (Ez[i, j + 1, k] - Ez[i, j, k]) / dyp[j]
                    dEy_dz = (Ey[i, j, k + 1] - Ey[i, j, k]) / dzp[k]
        ...
        for i in prange(Nx - 1):
            for j in range(Ny):
                for k in range(Nz - 1):
                    dEx_dz = (Ex[i, j, k + 1] - Ex[i, j, k]) / dzp[k]
                    dEz_dx = (Ez[i + 1, j, k] - Ez[i, j, k]) / dxp[i]
                    Hy[i, j, k] -= (dt / (_MU0 * mu_y[i, j, k])) * (dEx_dz - dEz_dx)
    else:
        # Nz=1 slice fast path (z-derivatives dropped)
        for i in prange(Nx):
            for j in range(Ny - 1):
                dEz_dy = (Ez[i, j + 1, 0] - Ez[i, j, 0]) / dyp[j]
                Hx[i, j, 0] -= (dt / (_MU0 * mu_x[i, j, 0])) * dEz_dy
        for i in prange(Nx - 1):
            for j in range(Ny):
                dEz_dx = (Ez[i + 1, j, 0] - Ez[i, j, 0]) / dxp[i]
    ...
    for i in prange(Nx - 1):
        for j in range(Ny - 1):
            for k in range(Nz):
                dEy_dx = (Ey[i + 1, j, k] - Ey[i, j, k]) / dxp[i]
                dEx_dy = (Ex[i, j + 1, k] - Ex[i, j, k]) / dyp[j]
...
def _update_E(Ex, Ey, Ez, Hx, Hy, Hz,
    ...
    # differenced axis, so it is divided by the DUAL width there. The updated cell
    # index is the diff's UPPER index (Hz[j]-Hz[j-1] at cell j), matching
    # update.py's dyd[:-1] broadcast where output index n uses dd[n]: here cell j
    # uses dyd[j-1] = yc[j]-yc[j-1]. Uniform grids keep the exact constant spacing
    # → bit-identical to the scalar divisor.
    if Nz > 1:
