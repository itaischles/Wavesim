...
- **Diagnostics** — point field, `|E|`/`|H|` magnitude, 2D snapshots (XY/XZ/YZ
  slice planes), 2D **Poynting** power-flow slices (`S = E × H` as a vector, same
  slice geometry), line-integral voltage (∫E·dl) and current (∮H·dl) monitors,
  and total energy. Snapshots are collocated off the staggered Yee points onto
  cell centres and H is averaged onto the E timebase, so every component of a
  frame shares one coordinate grid and one instant — exactly what a Poynting
  vector needs (frames are one cell shorter than the grid per in-plane axis — see
  `SnapshotMonitor`).
- **Visualisation** — grid/material/PML plots, field snapshots, animations,
  Poynting power-flow arrows over a flux-magnitude map, voltage/current time
  series, TEM-mode profiles, and full-3D helpers (orthogonal XY/XZ/YZ slice
  triptych + multi-plane animation).
