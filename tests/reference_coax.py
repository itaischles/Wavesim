...
def run(cell, fmax=FMAX, steps_factor=6.0, verbose=True):
    """One resolution: returns the §7 row plus the §8 V/I purity split."""
    grid, cx, cy = build(cell)
    ...
    wave = ws.GaussianPulse.for_fmax(fmax)
    ...
    sim = ws.Simulation(grid, sources=[], monitors=[vmon, imon],
                        backend='numba', pec_faces=('x0', 'x1', 'y0', 'y1'))
...
if __name__ == '__main__':
    cells = [float(a) * 1e-3 for a in sys.argv[1:]] or \
