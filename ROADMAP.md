...
The arrays run either as a thin `Nz=1` slice or as a **full 3D domain (`Nz>1`)** —
validated by a coaxial TEM mode exercising the 3D curl and z-face CPML on the
...
The 3D code paths work, but they are still gated by `Nz>1` branches and
`# 3D-UPGRADE:` guards written for the 2D-slice era. Promote 3D from "works when
you set `Nz>1`" to the default, well-exercised mode.

Checklist (search the codebase for `# 3D-UPGRADE:`):

- [x] z-face CPML allocated and active for `Nz>1` (`pml.py`).
- [x] z-face PEC supported (`pec.py` is already parametrised over all six faces).
- [x] Add a genuinely volumetric 3D validation — a rectangular PEC cavity
      matches 14 analytic resonances within 1.5%, 10 of them with a half-wave
      along `z` (`p>=1`), so 3D correctness no longer rests on the coax's
      extruded TEM mode alone.
- [x] 3D visualisation helpers in `viz.py` — `plot_field_slices_3d`
      ...
      multi-plane time animation).
      (Isosurfaces deferred — slice views cover current needs.)
- [x] Profile memory and runtime at representative 3D sizes.
...
- [ ] Remove / simplify the `Nz>1` z-derivative guards in `update.py` once 3D is
      ...
      benefits the 2D-slice runs and quick iteration. Note the Numba backend (§3)
...
`wavesim/sources.py`; see `docs/API_GUIDE.md` §6.
...
- [x] The functional core is untouched: the class only *orchestrates* the
      ...
      Verified bit-for-bit identical to a hand-written loop.
...
solver ran at <10% CPU utilisation (~1 of 12 cores), ~0.33 µs/cell-step, so a
96³ grid fit only ~600 steps in a 3-minute budget.
...
- **Parity**: bit-identical to NumPy (`max|diff| == 0`) on both the 2D-slice and
  full-3D paths (no parallel reductions, identical float64 arithmetic).
- **Speedup** (GTX-1660 box, 12 cores):
